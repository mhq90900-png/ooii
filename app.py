...
with st.spinner(f"🔄 جاري تحليل مصفوفة خيارات {selected_ticker} ({target_mode_text}) واكتشاف العقود المتجاوزة لمتوسط 50..."):
        ...
        if results:
            ...
            if 'strike_price' in df.columns:
                ...
                df['Liquidity Wall'] = np.select(
                    ...
                    ["🚨 Institutional Wall", "High Liquidity 🟢"],
                    default="Moderate 🟡"
                )
            ...
            with tab2:
                ...
                if not bullish_sma_df.empty:
                    display_cols = [c for c in ['ticker', 'contract_type', 'strike_price', 'expiration_date', 'DTE_days', 'Option Premium (\()', 'Option SMA 50 (\))', 'Trend Status', 'Open Interest (OI)', 'Market Bias'] if c in bullish_sma_df.columns]
